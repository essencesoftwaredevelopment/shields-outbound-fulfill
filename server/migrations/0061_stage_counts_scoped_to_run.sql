-- 0061_stage_counts_scoped_to_run.sql
-- Scope get_job_stage_counts completion counters to this run (jobs.created_at).
--
-- `include` re-runs reuse contacts whose *_completed_at stamps come from earlier
-- jobs, so the live cards counted months-old email-finder attempts as this run's
-- work (job 1789671032549-rjzmp4: "1,747 checked • 37.8% hit rate" while the
-- run had only imported 800 CSV emails and never called the finder). The
-- server-side reconcile (countJobStageStats runStartedAt) and the stage queues
-- already use jobs.created_at as the boundary; the RPC now matches them.
--
-- `emailFound` (any email, unstamped) is kept unscoped: it is the fallback for
-- legacy upload-included-email jobs whose emails were never stamped.
...
DECLARE
    ...
    result jsonb;
BEGIN
    SELECT id, agency_id, options, dedupe_stats, cost, created_at
      INTO j
      FROM jobs
     WHERE id = p_job_id;
    ...
    SELECT jsonb_build_object(
        'total', COUNT(*)::int,
        'founderDone', COUNT(*) FILTER (WHERE founder_find_completed_at >= j.created_at)::int,
        'founderFound', COUNT(*) FILTER (
            WHERE founder_find_completed_at >= j.created_at
              AND full_name IS NOT NULL AND BTRIM(full_name) <> ''
              AND LOWER(BTRIM(full_name)) <> 'not found'
        )::int,
        'emailDone', COUNT(*) FILTER (WHERE email_find_completed_at >= j.created_at)::int,
        -- Broader "any email on contact" (skip-finder imports + legacy readers).
        'emailFound', COUNT(*) FILTER (
            WHERE email IS NOT NULL AND BTRIM(email) <> ''
        )::int,
        -- Find-completed outcomes — found + notFound = emailDone (hit-rate denom).
        'emailFoundCompleted', COUNT(*) FILTER (
            WHERE email_find_completed_at >= j.created_at
              AND email IS NOT NULL AND BTRIM(email) <> ''
        )::int,
        'emailNotFound', COUNT(*) FILTER (
            WHERE email_find_completed_at >= j.created_at
              AND (email IS NULL OR BTRIM(email) = '')
        )::int,
        -- Verify / personalize outcomes only count contacts that hold an email, like
        -- countJobStageStats: a CSV status that landed on an email-less contact
        -- (its email belongs to another contact of this client) is not a lead.
        'verifyDone', COUNT(*) FILTER (
            WHERE email_verify_completed_at >= j.created_at
              AND email IS NOT NULL AND BTRIM(email) <> ''
        )::int,
        'valid', COUNT(*) FILTER (
            WHERE email_verify_completed_at >= j.created_at
        ...
        'personalizeDone', COUNT(*) FILTER (
            WHERE personalization_completed_at >= j.created_at
              AND email IS NOT NULL AND BTRIM(email) <> ''
        )::int,
    ...
    result := jsonb_build_object(
        ...
        'emailDiscovery', jsonb_build_object(
            ...
            -- skipEmailFinder imports: emailDone is 0; fall back to emails present.
            'found', CASE
                WHEN COALESCE((j.options->>'skipEmailFinder')::boolean, false)
                 AND COALESCE((contact_counts->>'emailDone')::int, 0) = 0
                THEN COALESCE((contact_counts->>'emailFound')::int, 0)
                ELSE COALESCE((contact_counts->>'emailFoundCompleted')::int, 0)
            END,
            'notFound', COALESCE((contact_counts->>'emailNotFound')::int, 0)
        ),
